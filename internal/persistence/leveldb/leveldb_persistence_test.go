...
func TestReadWriteStreams(t *testing.T) {
	...
	streams, err := p.ListStreams(ctx, nil, 0, persistence.SortDirectionDescending)
	assert.NoError(t, err)
	...
	streams, err = p.ListStreams(ctx, nil, 2, persistence.SortDirectionDescending)
	assert.NoError(t, err)
	assert.Len(t, streams, 2)
	assert.Equal(t, s3.ID, streams[0].ID)
	assert.Equal(t, s2.ID, streams[1].ID)

	streams, err = p.ListStreams(ctx, streams[1].ID, 2, persistence.SortDirectionDescending)
	assert.NoError(t, err)
...
func TestReadWriteListeners(t *testing.T) {
	...
	listeners, err := p.ListListeners(ctx, nil, 0, persistence.SortDirectionDescending)
	assert.NoError(t, err)
	...
	listeners, err = p.ListStreamListeners(ctx, nil, 0, persistence.SortDirectionDescending, sID1)
	assert.NoError(t, err)
	...
	assert.NoError(t, err)
	listeners, err = p.ListStreamListeners(ctx, nil, 0, persistence.SortDirectionDescending, sID2)
	assert.NoError(t, err)
...
func TestListStreamsBadJSON(t *testing.T) {
	...
	_, err = p.ListStreams(context.Background(), nil, 0, persistence.SortDirectionDescending)
	assert.Error(t, err)
...
func TestListListenersBadJSON(t *testing.T) {
	...
	_, err = p.ListListeners(context.Background(), nil, 0, persistence.SortDirectionDescending)
	assert.Error(t, err)

	_, err = p.ListStreamListeners(context.Background(), nil, 0, persistence.SortDirectionDescending, apitypes.NewULID())
	assert.Error(t, err)
