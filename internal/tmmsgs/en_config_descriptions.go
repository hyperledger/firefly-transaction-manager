...
var (
	...
	ConfigTransactionHandlerName = ffc("config.transactionHandler.name", "The name of the policy engine to use", i18n.StringType)
	...
	ConfigTransactionHandlerSimpleNonceStateTimeout      = ffc("config.transactionHandler.simple.nonceStateTimeout", "How old the most recently submitted transaction record in our local state needs to be, before we make a request to the node to query the next nonce for a signing address", i18n.TimeDurationType)
	ConfigTransactionHandlerSimpleFixedGasPrice          = ffc("config.transactionHandler.simple.fixedGasPrice", "A fixed gasPrice value/structure to pass to the connector", "Raw JSON")
	ConfigTransactionHandlerSimpleResubmitInterval       = ffc("config.transactionHandler.simple.resubmitInterval", "The time between warning and re-sending a transaction (same nonce) when a blockchain transaction has not been allocated a receipt", i18n.TimeDurationType)
	ConfigTransactionHandlerSimpleGasOracleEnabled       = ffc("config.transactionHandler.simple.gasOracle.mode", "The gas oracle mode", "'connector', 'restapi', 'fixed', or 'disabled'")
	ConfigTransactionHandlerSimpleGasOracleGoTemplate    = ffc("config.transactionHandler.simple.gasOracle.template", "REST API Gas Oracle: A go template to execute against the result from the Gas Oracle, to create a JSON block that will be passed as the gas price to the connector", i18n.GoTemplateType)
	ConfigTransactionHandlerSimpleGasOracleURL           = ffc("config.transactionHandler.simple.gasOracle.url", "REST API Gas Oracle: The URL of a Gas Oracle REST API to call", i18n.StringType)
	ConfigTransactionHandlerSimpleGasOracleProxyURL      = ffc("config.transactionHandler.simple.gasOracle.proxy.url", "Optional HTTP proxy URL to use for the Gas Oracle REST API", i18n.StringType)
	ConfigTransactionHandlerSimpleGasOracleMethod        = ffc("config.transactionHandler.simple.gasOracle.method", "The HTTP Method to use when invoking the Gas Oracle REST API", i18n.StringType)
	ConfigTransactionHandlerSimpleGasOracleQueryInterval = ffc("config.transactionHandler.simple.gasOracle.queryInterval", "The minimum interval between queries to the Gas Oracle", i18n.TimeDurationType)
