...
type Manager interface {
	...
	// - If streamConfirmations is true, then the returning events will include confirmations information
	// - If confirmation is set to non-zero a single event will be stream multiple times with different confirmation updates until the final confirmation
	//
	...
	GetAPIManagedEventStream(spec *apitypes.EventStream, listeners []*apitypes.Listener, streamConfirmations bool) (isNew bool, es eventapi.EventStream, err error)
