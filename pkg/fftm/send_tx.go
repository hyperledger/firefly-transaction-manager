...
func (m *manager) submitPreparedTX(ctx context.Context, reqID string, txHeaders *ffcapi.TransactionHeaders, gas *fftypes.FFBigInt, transactionData string) (*apitypes.ManagedTX, error) {
	...
	lockedNonce, err := m.assignAndLockNonce(ctx, reqID, txHeaders.From)
	if err != nil {
		return nil, err
	}
	...
	mtx := &apitypes.ManagedTX{
		Headers: apitypes.ManagedTXHeaders{
			RequestID:    reqID, // on input the request ID must be the namespaced operation ID
			TimeReceived: now,
			LastUpdate:   now,
		},
		SequenceID:         seqID,
